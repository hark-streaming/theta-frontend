        ...
        >
          <div v-if="showCountdown" style="position: absolute">
            <div
              ...
              "
            >
              <countdown :timestamp="scheduled" :key="scheduled" />
            </div>
          </div>
...
export default {
  ...
  head() {
    return {
      ...
      link: [{ rel: "canonical", href: `https://bitwave.tv/${this.name}` }],
      meta: [
        {
          property: "og:title",
          hid: "og:title",
  ...
  methods: {
    ...
    async streamDataChanged(data) {
      ...
      // Detect user going OFFLINE
        ...
        const canReset = () => {
          const atLiveEdge =
            $bw.player.liveTracker && $bw.player.liveTracker.atLiveEdge();
    ...
    const getChannelHydration = async () => {
      ...
      if (!channelData) {
        ...
        try {
          ...
          const streamDoc = await db.collection("streams").doc(streamer).get();
      ...
      try {
        ...
        // Process scheduled date
        const scheduled = data.scheduled ? new Date(data.scheduled) : null;
