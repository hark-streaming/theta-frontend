...
export default {
    ...
    methods: {
        ...
        //                 "NOTE: You have ignore users turned off"
        //             );
        //     }
        //     if (a.changeStatOnGraph)
        //         this.changeStatOnGraph(...a.changeStatOnGraph);
        //     if (a.chatServer) {
        ...
        async connectToChat() {
            ...
            bitwaveChat.socketReconnect = () => {
                this.connecting = false;
                this.loading = false;
            };
            bitwaveChat.socketError = () => {
                this.connecting = false;
                this.loading = true;
            };
        ...
        async scrollToBottom(force) {
            ...
            else console.warn("Could not find scroll container for chat.");
        },

        // async ignoreUser(user) {
        //     const actions = await this.$chatCommandParser.ignoreUser(
        //         user.toLowerCase()
        //     );
        ...
        //     const actions = await this.$chatCommandParser.unignoreUser(
        //         user.toLowerCase()
        //     );
        //     actions?.forEach((a) => this.executeAction(a));
        // },
        ...
        async authenticated(user) {
            ...
            if (user) {
                ...
                const tokenUpdated =
                    !this.userToken ||
                    this.userToken.token !== this.getChatToken;
                if (tokenUpdated) {
                    const tokenTroll = {
                        type: "troll",
                        token: this.getChatToken,
        ...
        //                 "ignoreChannelList",
        //                 this.ignoreChannelList
        //             );
        //         }

        //         // Check if reconnection required
        //         if (
        //             lastUser.avatar !== user.avatar ||
        ...
        //                 const tokenUpdated = !compareTokens(
        //                     oldToken,
        //                     this.getChatToken
        //                 );
        ...
        async hydrate() {
            ...
            if (!success) {
                this.$toast.error("Error hydrating chat!", {
                    icon: "error",
                    duration: 2000,
                    position: "top-right",
                });
            ...
            if (process.client) {
                this.$nextTick(() => {
                    if (this.$refs["chatmessages"])
                        ...
                        console.warn(
                            "Failed to find chat container after hydration"
                        );
                });
            }
        },
        ...
        async rcvMessageBulk(messages) {
            ...
            messages.forEach((m) => {
                // fireworks event
                if (m.type === "fireworks") {
                    if (!this.$utils.normalizedCompare(m.channel, this.page))
                        return;
                ...
                if (m.type === "delete") {
                    ...
                    this.messages = this.messages?.filter(
                        (message) => !m.ids.includes(message._id)
                    );
                    return;
                }
                ...
                // Notification Sounds
                if (this.notify)
                    if (pattern.test(m.message)) this.sound.play().then();
                ...
                if (this.getUseTts) {
                    ...
                    const currentChat = this.$utils.normalizedCompare(
                        m.channel,
                        this.username
                    );
                    const myChat = this.$utils.normalizedCompare(
                        m.channel,
                        this.page
                    );
                    ...
                    if (currentChat || myChat) {
                        const useTts = (username) => {
                            if (
                                this.ignoreList.find(
                                    (user) =>
                                        user.toLowerCase() ===
                                        m.username.toLowerCase()
                                )
                            )
                                return false; // Don't read ignored users
                            if (!this.getTrollTts && /troll:\w+/.test(username))
                                return false; // disables troll TTS
                            return true;
                        };
            ...
            if (!this.$refs["chatmessages"].showFAB) {
                ...
                if (this.messages.length > this.chatLimit)
                    this.messages.splice(
                        0,
                        this.messages.length - this.chatLimit
                    );
        ...
        //     const allFilters = this.messageFilters.reduce(
        //         maybeCompose,
        //         (a) => a
        //     );
        ...
        //     } else {
        //         this.insertMessage(
        //             "Invalid stat name (did you enable stat-tracking?)"
        //         );
        //     }
        // },
        ...
        //         this.messages.length - this.newMessageCount,
        ...
        //         await this.userStats.calculate.messageRate.everybody(
        //             newMessages
        //         );
        ...
        //     const data = this.userStats.stat.get(
        //         this.graphStat.user,
        //         this.graphStat.stat
        //     );
        //     if (data && data.values && data.values.length) {
                ...
                Object.freeze([
                    {
                        ...
                        avatar:
                            "https://cdn.bitwave.tv/static/img/glitchwave.gif",
                        message: message,
                        channel: this.page,
                    },
                ])
        ...
        //         console.log(
        //             `TTS Finished in ${(evt.elapsedTime / 1000).toFixed(
        //                 1
        //             )} seconds.`,
        //             evt
        //         );
        ...
        //             this.getTtsTimeout * 1000
        //         );
        //     }
        // },

        // showChatFireworks(text, bottomText) {
        //     this.$refs["fireworks"].start(text, bottomText);
        // },
    ...
    async mounted() {
        ...
        this.userStats.calculate.viewerCount = {
            total: () => {
                ...
                me.stat.value.set(
                    me.ALL_USER,
                    key,
                    this.getChannelViews(this.page)
                );
            },
        };
        ...
        this.userStats.calculate.hIndex = {
            total: async () => {
                ...
                const channels = streamers
                    .map((l) => this.getChannelViews(l.name))
                    .filter((c) => c !== 0);
                if (!channels) {
                    this.userStats.stat.value.set(
                        this.userStats.ALL_USER,
                        "hIndex",
                        0
                    );
                    return;
        ...
        this.userStats.defaultHistogramSettings = {
            create: true,
            size: this.getStatHistogramSize,
        };
        this.statInterval = setInterval(
            () => this.onChatStatTick(),
            this.getStatTickRate * 1000
        );
