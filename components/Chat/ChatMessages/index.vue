<template>
    <v-flex id="inner-chat" fill-height>
        <div id="chat-scroll" ref="scroller">
            <!-- It is much quicker to simply directly render into chat and re-use by index -->
            <div :class="{ dense: highDensity }">
        ...
        <!-- <div>
            ...
            >
                <lazy-chat-message-menu
                    ...
                    :user-styling="{
                        color: selectedChatMessage.userStyling
                            ? selectedChatMessage.userStyling
                            : '#9e9e9e',
                    }"
...
export default {
    ...
    methods: {
        ...
        checkIfBottom() {
            if (!this.messages || this.messages.length === 0 || this.atBottom) {
                ...
                const lastMessageHeight = this.chatContainer.lastElementChild
                    .clientHeight;
                // const lastMessageHeight = document.querySelector("#chat-scroll > div:last-child").clientHeight;
                return (
                    scrollTop + clientHeight >= scrollHeight - lastMessageHeight
                );
            }
        },
        ...
        jumpToBottom(scrollGoal) {
            this.chatContainer.scrollTop = scrollGoal
                ? scrollGoal
                : this.chatContainer.scrollHeight + 750;
            this.atBottom = true;
        },
        ...
        getTime(timestamp) {
            ...
            return {
                ...
                long: `sent: ${moment(timestamp).format(
                    "YYYY.MM.DD HH:mm:ss"
                )}`,
            };
        },
        ...
        onScrollDown(scrollPosition) {
            if (
                scrollPosition + this.chatContainer.clientHeight >
                this.chatContainer.scrollHeight - 250
            ) {
                this.showFAB = false;
            }
        },
    ...
    async mounted() {
        ...
        if (this.$route.path.startsWith("/chat")) {
            this.routePrefix = this.$route.path === "/chat" ? "/chat/" : "";
        } else {
            this.routePrefix = "/";
        }
    },
    ...
    updated(){     
        ...
        if(this.priorMessageCount < this.messages.length && this.atBottom){
            ...
            this.priorMessageCount = this.messages.length;
        }
