    ...
    >
      ...
      <slot name="minioverlay">
        <div v-if="docked" class="d-flex align-center detach-overlay">
          ...
          <!-- Close Button -->
          <v-btn color="white" text icon pa-0 @click="setDetach(false)">
  ...
  methods: {
    playerInitialize() {
      ...
      this.player = videojs("streamplayer", {
        poster: this.poster,
        sources: [
         { src: this.source.url, type: this.source.type } 
          /*{
            src: "YOUR_VIDEO_URL",
      ...
      const handleVolumeScroll = (event) => {
        ...
        // Scroll 'up'
        if (event.deltaY < 0) this.player.volume(Math.min(1.0, vol + 0.05));
        // Scroll 'down'
        if (event.deltaY > 0) this.player.volume(Math.max(0.0, vol - 0.05));
      };
