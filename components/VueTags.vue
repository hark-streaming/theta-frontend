<template>
  <div class="tags" ref="tags">
    ...
    >
      ...
      <div class="tags__list" v-if="tagListActive">
  ...
  computed: {
    filteredList() {
      let list = this.all.filter(
        (tag) => tag.name.toLowerCase().indexOf(this.search.toLowerCase()) >= 0
      );

      if (
        this.tagCreationEnabled &&
        this.search.length > 0 &&
        list.every((item) => item.name !== this.search)
      ) {
        if (!this.newTagColor) {
  ...
  methods: {
    ...
    tagSelected(index) {
      return this.active.some(
        (element) => element.slug === this.filteredList[index].slug
      );
    },
    handleEnterKey() {
